...
public class EnemyGenerate : MonoBehaviour
{
    ...
    private IEnumerator createEnemy()
    {
        for (int j = 0; j < generateNum; j++)
        {
            ...
            {
                var enemyJunior = Instantiate(generatedEnemy[i], generateSpawn[i].transform.position, Quaternion.identity);
                enemyJunior.transform.SetParent(gameObject.transform);
            }
